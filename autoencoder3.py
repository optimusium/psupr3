...
raw_data2 = pd.read_excel('td10.xlsx',sheet_name = "td10")
raw_data3 = pd.read_excel('td11.xlsx',sheet_name = "td11")
#split into resistance input data and label output data
sdarray2=raw_data2.drop(["saw","grind","dielectric"],axis=1)
sdarray3=raw_data3.drop(["saw","grind","dielectric"],axis=1)
...
raw_label3=raw_data3.saw+2*raw_data3.grind+4*raw_data3.dielectric

#3.2 8:2 train/test data split
sdarray_train2,sdarray_test3,label_train2,label_test3 = train_test_split(sdarray2,raw_label2,test_size = 0.01)
sdarray_train3,sdarray_test2,label_train3,label_test2 = train_test_split(sdarray3,raw_label3,test_size = 0.99)
...
def createModel():
    ...
    x=Lambda(lambda x: x - 0.15)(x)
    x=Activation("relu")(x)
    outputs=Lambda(lambda x: x + 0.15)(x)
    #x           = Lambda(lambda x: x ** 2)(x)
